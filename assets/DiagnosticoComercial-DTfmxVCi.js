...
`;function Z(a,s,o,c){const l=new Date().toLocaleDateString("es-CL",{year:"numeric",month:"long",day:"numeric"});return`<div class="cab">
    ...
    <div class="barra"></div>
  </div>`}function ee(a){return`<p class="pie">coproactiva · Comunidades que funcionan. — Documento confidencial — ${a}</p>`}function ge(a){var b;const{prospecto:s,diagnostico:o,areas:c,notasAreas:l,lineaElegida:k,logo:g}=a,y=new Date().toLocaleDateString("es-CL",{year:"numeric",month:"long",day:"numeric"}),n=X[k]??X.L2,N=K(o.nivel==="Crítico"?10:o.nivel==="En riesgo"||o.globalScore>=80&&o.hallazgosCriticos.length?45:o.globalScore),q=[...c].filter(d=>d.pct!=null).sort((d,x)=>d.pct-x.pct),z={};for(const d of o.hallazgosCriticos)(z[b=d.area]??(z[b]=[])).push(d.texto);const f=`<div class="hoja">
    ${Z(1,2,s,g)}
    <div class="cuerpo">
      <div class="seccion-titulo">Resultado del diagnóstico</div>
      <div class="resultado" style="background:${N.fondo};border-color:${N.borde}">
        <div class="aro"><div class="n" style="color:${N.texto}">${o.globalScore}</div><div class="s">%</div></div>
        <div class="txt">
          <div class="nivel" style="color:${N.texto}">${C(o.nivel)}</div>
          <p style="color:${N.texto}">${ve(o)}</p>
        </div>
      </div>

      <div class="seccion-titulo">Comparativo por área — de menor a mayor cumplimiento</div>
      ${q.map(d=>{const x=K(d.pct);return`<div class="area-fila">
          <div class="nombre">${C(d.label)}</div>
          <div class="barra-fondo">
            <div class="barra-rellena" style="width:${d.pct}%;background:${x.borde}"></div>
            <span class="pct" style="${d.pct>20?"right:4mm;color:#fff":`left:${d.pct}%;margin-left:2mm;color:${x.texto}`}">${d.pct}%</span>
          </div>
          <div class="estado" style="background:${x.fondo};color:${x.texto}">${x.etiqueta}</div>
        </div>`}).join("")}

      <div class="seccion-titulo">Observaciones por área</div>
      <div class="obs-grid">
        ${c.map(d=>{const x=K(d.pct),h=l==null?void 0:l[d.id];return`<div class="obs-tarjeta" style="border-left-color:${x.borde}">
            <div class="cab2">
              <span class="nombre">${C(d.label)}</span>
              ${d.pct!=null?`<span class="pct" style="color:${x.texto}">${d.pct}% · ${x.etiqueta}</span>`:""}
            </div>
            <div class="texto">${h?C(h):"<em>Sin observaciones registradas.</em>"}</div>
          </div>`}).join("")}
      </div>
    </div>
    ${ee(y)}
    ...
    <div class="cuerpo">
      ${o.hallazgosCriticos.length?`
        ...
        ${Object.entries(z).map(([d,x])=>`
          <div class="hallazgo-area">${C(d)}</div>
          ${x.map(h=>`<div class="hallazgo"><div class="x">×</div><div class="t">${C(h)}</div></div>`).join("")}
        `).join("")}
      `:""}
...
<body>${f}${_}</body>
</html>`}function ve(a){var s;if(a.nivel==="Estable")return"La comunidad presenta una gestión ordenada en todas las áreas. CoproActiva puede optimizarla de manera continua.";if(a.nivel==="Estable con observaciones"){const o=[];return(s=a.areasBajo60)!=null&&s.length&&o.push("focos de riesgo en: "+a.areasBajo60.map(c=>{var l;return((l=c.area)==null?void 0:l.label)??c.label}).join(", ")),a.hasCritFail&&o.push(`${a.hallazgosCriticos.length} ítem(s) crítico(s) sin resolver`),`El puntaje global es satisfactorio, pero se identifican: ${o.join(" y ")}. Estos aspectos requieren atención aunque el resultado global sea positivo.`}return a.nivel==="En riesgo"?"Existen aspectos que requieren intervención moderada o incumplimientos en ítems críticos. Sin acción oportuna pueden derivar en problemas administrativos, legales o financieros de mayor envergadura.":"Se detectan deficiencias graves en una o más áreas. Se requiere intervención urgente para evitar consecuencias legales y financieras significativas."}const A=[{id:"legal",label:"Legal"},{id:"financiero",label:"Financiero"},{id:"laboral",label:"Laboral",condArea:"trabajadores"},{id:"tecnico",label:"Técnico"},{id:"seguridad",label:"Seguridad"},{id:"documental",label:"Documental"}],ae={3:"Crítico",2:"Importante",1:"Básico"},fe=[{valor:0,etiqueta:"No cumple",clase:"critico"},{valor:.25,etiqueta:"No sabe",clase:"neutro"},{valor:.5,etiqueta:"Parcial",clase:"observacion"},{valor:1,etiqueta:"Cumple",clase:"cumple"}],be={L1:{label:"Administración integral",desc:"La comunidad tiene base. CoproActiva asume la administración completa y optimiza los procesos existentes."},L2:{label:"Nueva administración",desc:"La comunidad necesita restructuración. CoproActiva ordena cada área desde cero y regulariza la gestión."},L3:{label:"Asesoría externa",desc:"Diagnóstico y orientación sin cambio de administración. Apoyo técnico al comité vigente."}};function he(a,s,o){return a<40?"L3":a>=75&&s.length===0&&!o?"L1":"L2"}function Se({id:a}){const s=me(),[o,c]=$.useState(null),[l,k]=$.useState(null),[g,y]=$.useState(null),[n,N]=$.useState({}),[q,z]=$.useState({}),[f,_]=$.useState(null),[b,d]=$.useState("contexto"),[x,h]=$.useState(null),[ie,P]=$.useState(!1);async function oe(){var v;h(null);const{data:i,error:t}=await w.from("controles").select("*").eq("id",a).maybeSingle();if(t)return h(t.message);if(!i)return h("Este diagnóstico no existe o no tienes acceso.");if(!i.prospecto_id)return h("Este diagnóstico no está ligado a un prospecto del Pipeline.");c(i);const[r,m,j]=await Promise.all([i.prospecto_id?w.from("prospectos").select("*").eq("id",i.prospecto_id).maybeSingle():Promise.resolve({data:null,error:null}),w.from("control_items").select("*").eq("control_id",a).order("orden"),w.from("diagnosticos_resultado").select("*").eq("control_id",a).maybeSingle()]);if(m.error)return h(m.error.message);if(r.error||!r.data)return h("No tienes acceso al prospecto de este diagnóstico.");k(r.data),y(m.data??[]),_(j.data??null);const p={};for(const S of m.data??[])S.respuesta&&typeof S.respuesta=="object"&&"valor"in S.respuesta&&(p[S.id]=S.respuesta.valor);N(p),z(((v=j.data)==null?void 0:v.notas_areas)??{})}$.useEffect(()=>{oe()},[a]);const D=$.useMemo(()=>{const i=new Map;for(const t of g??[])i.has(t.grupo)||i.set(t.grupo,[]),i.get(t.grupo).push(t);return i},[g]),te=$.useMemo(()=>{var t;const i=new Map;for(const r of g??[]){const m=(t=r.config)==null?void 0:t.clave;m&&i.set(m,r)}return i},[g]),Q=$.useMemo(()=>(g??[]).find(i=>i.grupo==="contexto"&&i.texto==="Trabajadores contratados"),[g]),R=Q?Number(n[Q.id]||0):0;function Y(i){if(i==="trabajadores")return R>0;if(i==="menosDiez")return R>0&&R<10;if(i==="diezOMas")return R>=10;const t=te.get(i);return t?!!n[t.id]:!1}function T(i){var r;const t=(r=i.config)==null?void 0:r.condicion;return!(t&&!Y(t))}function B(i){return i.condArea?Y(i.condArea):!0}function F(i){var p;if(!B(i))return null;const t=(D.get(i.id)??[]).filter(T);if(t.length===0)return null;let r=0,m=0,j=0;for(const v of t){const S=((p=v.config)==null?void 0:p.peso)??1;m+=S;const u=n[v.id];u!=null&&(r+=S*Number(u)),S===3&&(u===0||u===.25)&&(j+=1)}return m===0?null:{pct:Math.round(r/m*100),criticosFallidos:j,evaluados:t.filter(v=>n[v.id]!=null).length,total:t.length}}const E=$.useMemo(()=>{var S;const i=A.map(u=>({area:u,score:F(u)})).filter(u=>u.score),t=i.length?Math.round(i.reduce((u,O)=>u+O.score.pct,0)/i.length):0,r=[];for(const u of A)if(B(u))for(const O of(D.get(u.id)??[]).filter(T)){if((((S=O.config)==null?void 0:S.peso)??1)!==3)continue;const U=n[O.id];(U===0||U===.25)&&r.push({area:u.label,texto:O.texto,tipo:U===0?"no_cumple":"no_sabe"})}const m=r.length>0,j=i.filter(u=>u.score.pct<60),p=i.filter(u=>u.score.pct<40);let v;return t>=80&&j.length===0&&!m?v="Estable":t>=80?v="Estable con observaciones":t>=50?v="En riesgo":v="Crítico",{globalScore:t,hallazgosCriticos:r,hasCritFail:m,areasBajo60:j,areasRed:p,nivel:v,linea:he(t,p,m)}},[g,n]);function V(i,t){N(r=>({...r,[i]:r[i]===t?void 0:t}))}async function J(i){P(!0),h(null);const t=(g??[]).map(p=>({id:p.id,respuesta:{valor:n[p.id]??null},evaluado_en:n[p.id]!==void 0?new Date().toISOString():p.evaluado_en}));for(const p of t){const{error:v}=await w.from("control_items").update({respuesta:p.respuesta,evaluado_en:p.evaluado_en}).eq("id",p.id);if(v)return P(!1),h(v.message)}const r={control_id:a,score:E.globalScore,nivel:E.nivel,linea_sugerida:E.linea,linea_elegida:(f==null?void 0:f.linea_elegida)??E.linea,hallazgos_criticos:E.hallazgosCriticos,notas_areas:q},{data:m,error:j}=f?await w.from("diagnosticos_resultado").update(r).eq("control_id",a).select().single():await w.from("diagnosticos_resultado").insert(r).select().single();if(j||!m)return P(!1),h((j==null?void 0:j.message)||"No se pudo guardar el resultado.");if(_(m),i&&(o==null?void 0:o.estado)!=="enviado"){const{error:p}=await w.from("controles").update({estado:"enviado",enviado_en:new Date().toISOString()}).eq("id",a);if(p)return P(!1),h(p.message);c(v=>({...v,estado:"enviado",enviado_en:new Date().toISOString()}))}P(!1)}async function se(i){if(!f)return;P(!0);const{data:t,error:r}=await w.from("diagnosticos_resultado").update({linea_elegida:i}).eq("control_id",a).select().single();if(P(!1),r||!t)return h((r==null?void 0:r.message)||"No se pudo guardar la línea elegida.");_(t)}function ne(){const i=(f==null?void 0:f.linea_elegida)??E.linea,t=A.map(m=>{var j;return{id:m.id,label:m.label,pct:((j=F(m))==null?void 0:j.pct)??null}}),r=ge({prospecto:l,diagnostico:E,areas:t,notasAreas:q,lineaElegida:i,logo:"/cotizador-coproactiva/logo-coproactiva.svg"});pe(r)||h("El navegador bloqueó la ventana del informe. Permite las ventanas emergentes para este sitio.")}if($.useEffect(()=>{var i;(i=document.querySelector(".pestanas .activo"))==null||i.scrollIntoView({block:"nearest",inline:"nearest"})},[b]),x)return e.jsx("div",{className:"pantalla",children:e.jsx("div",{className:"cuerpo",children:e.jsx("div",{className:"aviso aviso-critico",children:x})})});if(!o||!l||g===null)return e.jsx("p",{className:"cargando",children:"Cargando…"});const re=D.get("contexto")??[],le=D.get("instalaciones")??[],L=["contexto","instalaciones",...A.map(i=>i.id),"revision"],I=L.indexOf(b),G=A.filter(B).flatMap(i=>(D.get(i.id)??[]).filter(T)),W=G.filter(i=>n[i.id]!=null).length,ce=G.length?Math.round(W/G.length*100):0;function de(i){const t=A.find(m=>m.id===i);if(!t||!B(t))return!1;const r=F(t);return!!(r&&r.evaluados===r.total)}return e.jsxs("div",{className:"pantalla pantalla-angosta",children:[e.jsxs("header",{className:"encabezado",children:[e.jsxs("div",{className:"fila",style:{marginBottom:8},children:[e.jsx("button",{className:"boton boton-texto",style:{padding:"4px 8px 4px 0"},onClick:()=>s("/pipeline"),children:"‹ Pipeline"}),e.jsx("span",{className:"crece"}),o.estado==="enviado"&&e.jsx("span",{className:"chip chip-cumple",children:"Enviado"})]}),e.jsx("h1",{className:"h3",children:l.nombre_condominio}),e.jsxs("p",{className:"chico apagado",style:{margin:"3px 0 12px"},children:["Diagnóstico comercial",l.comuna?` · ${l.comuna}`:""]}),e.jsxs("div",{className:"fila",style:{marginBottom:5},children:[e.jsx("span",{className:"etiqueta-campo crece",style:{margin:0},children:"Avance"}),e.jsxs("span",{className:"etiqueta-campo",style:{margin:0,color:"var(--texto-titulo)"},children:[W," de ",G.length]})]}),e.jsx("div",{className:"barra",children:e.jsx("div",{style:{width:ce+"%"}})})]}),e.jsxs("div",{className:"cuerpo",children:[o.estado==="enviado"&&e.jsx("div",{className:"aviso",style:{marginBottom:14},children:"Este diagnóstico ya fue enviado. Los cambios se siguen guardando si lo editas."}),e.jsx("nav",{className:"pestanas","aria-label":"Pasos del diagnóstico",children:L.map(i=>e.jsx("button",{type:"button",className:(i===b?"activo":"")+(de(i)?" completo":""),"aria-current":i===b?"step":void 0,onClick:()=>d(i),children:xe(i)},i))}),b==="contexto"&&e.jsx(je,{prospecto:l,items:re,respuestas:n,onResponder:V}),b==="instalaciones"&&e.jsx(Ne,{items:le,respuestas:n,onResponder:V}),A.map(i=>b===i.id&&e.jsx(ze,{area:i,items:D.get(i.id)??[],respuestas:n,notas:q,setNotas:z,itemAplica:T,areaAplica:B,scoreArea:F,onResponder:V},i.id)),b==="revision"&&e.jsx($e,{diagnostico:E,resultado:f,guardando:ie,onGuardar:()=>J(!1),onEnviar:()=>J(!0),onElegirLinea:se,onGenerarPdf:ne}),e.jsxs("nav",{className:"pasos","aria-label":`Paso ${I+1} de ${L.length}`,style:{marginTop:20},children:[e.jsx("button",{type:"button",className:"boton boton-secundario",disabled:I===0,onClick:()=>d(L[I-1]),children:"‹ Anterior"}),e.jsx("div",{className:"conteo",children:e.jsxs("span",{children:[I+1," de ",L.length]})}),e.jsx("button",{type:"button",className:"boton",style:I===L.length-1?{visibility:"hidden"}:void 0,onClick:()=>d(L[I+1]),children:"Siguiente ›"})]})]})]})}function xe(a){var o;return{contexto:"Datos",instalaciones:"Instalaciones",revision:"Revisión"}[a]??((o=A.find(c=>c.id===a))==null?void 0:o.label)??a}function M({children:a,extra:s}){return e.jsxs("div",{className:"fila",style:{marginBottom:10},children:[e.jsx("h2",{className:"etiqueta-grupo crece",style:{margin:0},children:a}),s]})}function je({prospecto:a,items:s,respuestas:o,onResponder:c}){return e.jsxs("section",{className:"diag-seccion",children:[e.jsx(M,{children:"Datos de la comunidad"}),e.jsxs("div",{className:"tarjeta diag-prospecto",children:[e.jsx("p",{className:"micro",style:{margin:0},children:"Estos datos vienen del prospecto y se editan desde el Pipeline."}),e.jsxs("div",{className:"diag-datos-prospecto",children:[e.jsx(H,{etiqueta:"Condominio",valor:a.nombre_condominio}),e.jsx(H,{etiqueta:"Dirección",valor:[a.direccion,a.comuna].filter(Boolean).join(", ")||"—"}),e.jsx(H,{etiqueta:"Unidades",valor:a.unidades??"—"}),e.jsx(H,{etiqueta:"Contacto",valor:a.nombre_contacto?`${a.nombre_contacto}${a.cargo_contacto?" · "+a.cargo_contacto:""}`:"—"})]})]}),e.jsx("div",{className:"diag-lista diag-lista-doble",children:s.map(l=>e.jsx(ye,{item:l,valor:o[l.id],onResponder:c},l.id))})]})}function ye({item:a,valor:s,onResponder:o}){var c,l,k,g;if(a.tipo_ingreso==="seleccion"){const y=((c=a.config)==null?void 0:c.opciones)??[];return e.jsxs("div",{className:"campo",children:[e.jsx("label",{className:"etiqueta-campo",htmlFor:"diag-"+a.id,children:a.texto}),e.jsxs("select",{id:"diag-"+a.id,value:s??"",onChange:n=>o(a.id,n.target.value||void 0),children:[e.jsx("option",{value:"",children:"Sin definir"}),y.map(n=>e.jsx("option",{value:n.valor,children:n.etiqueta},n.valor))]})]})}return a.tipo_ingreso==="numero"?e.jsxs("div",{className:"campo",children:[e.jsx("label",{className:"etiqueta-campo",htmlFor:"diag-"+a.id,children:a.texto}),e.jsxs("div",{className:"campo-medida",children:[e.jsx("input",{id:"diag-"+a.id,type:"number",inputMode:"numeric",min:((l=a.config)==null?void 0:l.min)??0,value:s??"",placeholder:"0",onChange:y=>o(a.id,y.target.value===""?void 0:Number(y.target.value))}),((k=a.config)==null?void 0:k.unidad)&&e.jsx("span",{className:"unidad",children:a.config.unidad})]}),((g=a.config)==null?void 0:g.hint)&&e.jsx("span",{className:"micro",children:a.config.hint})]}):e.jsxs("label",{className:"marca diag-marca"+(s?" activa":""),children:[e.jsx("input",{type:"checkbox",checked:!!s,onChange:y=>o(a.id,y.target.checked)}),e.jsx("span",{children:a.texto})]})}function Ne({items:a,respuestas:s,onResponder:o}){return e.jsxs("section",{className:"diag-seccion",children:[e.jsx(M,{children:"Instalaciones y equipamiento"}),e.jsx("p",{className:"micro",style:{margin:"-4px 0 10px"},children:"Marca lo que tiene el edificio: define qué puntos aplican en el diagnóstico."}),e.jsx("div",{className:"tarjeta lista-marcas diag-marcas",children:a.map(c=>e.jsxs("label",{className:"marca"+(s[c.id]?" activa":""),children:[e.jsx("input",{type:"checkbox",checked:!!s[c.id],onChange:l=>o(c.id,l.target.checked)}),e.jsx("span",{children:c.texto})]},c.id))})]})}function ze({area:a,items:s,respuestas:o,notas:c,setNotas:l,itemAplica:k,areaAplica:g,scoreArea:y,onResponder:n}){const N=g(a),q=y(a);return N?e.jsxs("section",{className:"diag-seccion",children:[e.jsx(M,{extra:q&&e.jsxs("span",{className:"chip",children:[q.pct,"% · ",q.evaluados," de ",q.total]}),children:a.label}),e.jsx("div",{className:"diag-lista",children:s.filter(k).map(z=>{var _;const f=(_=z.config)==null?void 0:_.peso;return e.jsxs("article",{className:"tarjeta punto",children:[e.jsxs("div",{className:"fila",style:{alignItems:"flex-start",gap:10,marginBottom:12},children:[e.jsx("p",{className:"crece",style:{margin:0},children:z.texto}),ae[f]&&e.jsx("span",{className:"chip"+(f===3?" chip-critico":""),children:ae[f]})]}),e.jsx("div",{className:"selector",children:fe.map(b=>e.jsx("button",{type:"button",className:b.clase,"aria-pressed":o[z.id]===b.valor,onClick:()=>n(z.id,b.valor),children:b.etiqueta},b.valor))})]},z.id)})}),e.jsxs("div",{className:"campo",style:{marginTop:14},children:[e.jsx("label",{className:"etiqueta-campo",htmlFor:"notas-"+a.id,children:"Observaciones del área"}),e.jsx("textarea",{id:"notas-"+a.id,value:c[a.id]??"",onChange:z=>l(f=>({...f,[a.id]:z.target.value}))})]})]}):e.jsxs("section",{className:"diag-seccion",children:[e.jsx(M,{children:a.label}),e.jsxs("div",{className:"vacio",children:["Esta comunidad no tiene personal contratado, por lo que el área ",a.label," no se evalúa."]})]})}function H({etiqueta:a,valor:s}){return e.jsxs("div",{children:[e.jsx("span",{className:"etiqueta-campo",children:a}),e.jsx("span",{className:"chico diag-dato",children:s})]})}function $e({diagnostico:a,resultado:s,guardando:o,onGuardar:c,onEnviar:l,onElegirLinea:k,onGenerarPdf:g}){const y=(s==null?void 0:s.linea_elegida)??a.linea;return e.jsxs("section",{className:"diag-seccion",children:[e.jsx(M,{children:"Resultado"}),e.jsxs("div",{className:"tablero diag-tablero",children:[e.jsxs("div",{className:a.globalScore>=80?"ok":a.globalScore>=50?"alerta":"critico",children:[e.jsxs("p",{className:"n",children:[a.globalScore,"%"]}),e.jsx("p",{className:"r",children:"Puntaje global"})]}),e.jsxs("div",{children:[e.jsx("p",{className:"n texto",children:a.nivel}),e.jsx("p",{className:"r",children:"Nivel"})]}),e.jsxs("div",{className:a.hasCritFail?"critico":"",children:[e.jsx("p",{className:"n",children:a.hallazgosCriticos.length}),e.jsx("p",{className:"r",children:"Hallazgos críticos"})]}),e.jsxs("div",{children:[e.jsx("p",{className:"n",children:a.areasBajo60.length}),e.jsx("p",{className:"r",children:"Áreas bajo 60%"})]})]}),a.hallazgosCriticos.length>0&&e.jsxs("div",{className:"diag-bloque",children:[e.jsx(M,{children:"Hallazgos críticos"}),e.jsx("div",{className:"diag-lista",children:a.hallazgosCriticos.map((n,N)=>e.jsxs("div",{className:"tarjeta punto",children:[e.jsxs("div",{className:"fila",style:{marginBottom:6},children:[e.jsx("span",{className:"etiqueta-campo crece",style:{margin:0},children:n.area}),e.jsx("span",{className:"chip"+(n.tipo==="no_cumple"?" chip-critico":""),children:n.tipo==="no_cumple"?"No cumple":"No sabe"})]}),e.jsx("p",{className:"chico",style:{margin:0},children:n.texto})]},N))})]}),e.jsxs("div",{className:"diag-bloque",children:[e.jsx(M,{children:"Línea de servicio"}),e.jsx("div",{className:"diag-lineas",children:Object.entries(be).map(([n,N])=>e.jsxs("button",{type:"button",className:"tarjeta diag-linea","aria-pressed":y===n,onClick:()=>k(n),disabled:!s,children:[e.jsxs("span",{className:"fila",style:{gap:8},children:[e.jsx("span",{className:"dato-chico crece",children:N.label}),a.linea===n&&e.jsx("span",{className:"chip chip-pendiente",children:"Sugerida"})]}),e.jsx("span",{className:"micro",children:N.desc})]},n))}),!s&&e.jsx("p",{className:"micro",style:{margin:"8px 0 0"},children:"Guarda el diagnóstico para poder elegir la línea."})]}),e.jsxs("div",{className:"fila-botones",style:{marginTop:20},children:[e.jsx("button",{type:"button",className:"boton boton-secundario boton-movil crece",onClick:c,disabled:o,children:"Guardar avance"}),e.jsx("button",{type:"button",className:"boton boton-movil crece",onClick:l,disabled:o,children:"Guardar y enviar"})]}),s&&e.jsx("button",{type:"button",className:"boton boton-secundario boton-movil boton-ancho",style:{marginTop:8},onClick:g,children:"Ver informe / Guardar PDF"})]})}export{Se as default};
